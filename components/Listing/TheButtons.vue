<template>
  <div class="listings-section_actions card-actions" v-if="!isAdmin || checkRoute()">
    ...
    <button class="btn" @click.prevent="removeFromFavorites(listing.id)" v-if="checkFavoriteListing(listing.favorites)">
    ...
    <button class="btn" @click.prevent="addToFavorites(listing.id)" v-else>
...
export default{
    computed:{
        ...mapState(useAuthStore,['isAdmin','user']),
    },
    props:{
      listing:{},
    },
